<?PHP
/*
 * $Id: TIFFFaxDecoder.php,v 1.2 2005/11/14 18:24:13 mstaylor Exp $
 * $Name:  $
 *
...
class TIFFFaxDecoder
{
    ...
    private $data = NULL;//a byte array
    private $w = 0;
    private $h = 0;
    private $fillOrder = 0;
    ...
    private $prevChangingElems = NULL;//array
    private $currChangingElems = NULL;//array
    ...
    protected static $flipTable = NULL;

    // The main 10 bit white runs lookup table
    protected static $white = array(
        // 0 - 7
    ...
    protected static $additionalMakeup = array(
    ...
    protected static $initBlack = array(
        // 0 - 7
    ...
    //
    protected static $twoBitBlack = array(292, 260, 226, 226);   // 0 - 3

    // Main black run table, using the last 9 bits of possible 13 bit code
    protected static $black = array(
        // 0 - 7
        62,     62,     30,     30,     0,      0,      0,      0,
        // 8 - 15
        0,      0,      0,      0,      0,      0,      0,      0,
    ...
    protected static $twoDCodes= NULL;//byte array
    ...
    {
        if(TIFFFaxDecoder::$initialized == FALSE)
        {
            TIFFFaxDecoder::$twoDCodes = itextphp_bytes_create(array(
            // 0 - 7
           ...
           TIFFFaxDecoder::$flipTable = itextphp_bytes_create(array(
           ...
           TIFFFaxDecoder::$initialized = TRUE;
        }
    }
    ...
    * @param h
    */
    private function construct3args($fillOrder, $w, $h) {
        ...
        $this->bitPointer = 0;
        $this->bytePointer = 0;
        $this->prevChangingElems = array($w);
        $this->currChangingElems = array($w);
    }


    // One-dimensional decoding methods

    public function decode1D($buffer, $compData, $startX, $height) {
        $this->data = $compData;

        $lineOffset = 0;
        $scanlineStride = ($w + 7)/8;
    ...
    public function decodeNextScanline($buffer, $lineOffset, $bitOffset) {
        $bits = 0;
        $code = 0;
        $isT = 0;
        $current = 0;
    ...
    public void decode2D($buffer, $compData, $startX, $height, $tiffT4Options) {
        $this->data = $compData;
        $compression = 3;
        ...
        $oneD = (integer)($tiffT4Options & 0x01);
        $uncompressedMode = (integer)(($tiffT4Options & 0x02) >> 1);
        $fillBits = (integer)(($tiffT4Options & 0x04) >> 2);
        ...
        for ($lines = 1; $lines < $height; $lines++) {
            ...
            if (readEOL(FALSE) == 0) {
                ...
                while ($bitOffset < $w) {
                    ...
                    $entry = (integer)(itextphp_bytes_getIntValue(TIFFFaxDecoder::$twoDCodes, $entry) & 0xff);
    ...
    public function decodeT6($buffer, $compData, $startX, $height, $tiffT6Options) {
        $this->data = $compData;
        $compression = 4;
        ...
        $uncompressedMode = (integer)(($tiffT6Options & 0x02) >> 1);
    ...
    private function setToBlack($buffer, $lineOffset, $bitOffset, $numBits) {
        ...
        if ($shift > 0) {
            $maskVal = 1 << (7 - $shift);
            //$val = itextphp_bytes_getIntValue($buffer, $byteNum);
            while ($maskVal > 0 && $bitNum < $lastBit) {
                itextphp_bytes_bitwiseAssign($buffer, $byteNume, $maskVal);
                $maskVal >>= 1;
                ++$bitNum;
            }
            //buffer[byteNum] = val;
        ...
        while ($bitNum < $lastBit - 7) {
            itextphp_bytes_write($buffer, $byteNum++, itextphp_bytes_createfromInt(255), 0);
            $bitNum += 8;
        }
        ...
        while ($bitNum < $lastBit) {
            $byteNum = $bitNum >> 3;
            itextphp_bytes_bitwiseAssign($buffer, $byteNum, 1 << (7 - ($bitNum & 0x7)));
            ++$bitNum;
        }
    }
    ...
    // Returns run length
    private function decodeWhiteCodeWord() {
        $current = 0;
        $entry = 0;
        $bits = 0;
        $isT = 0;
        ...
        while ($isWhite == TRUE) {
            $current = nextNBits(10);
            $entry = TOFFFaxDecoder::$white[$current];
    ...
    // Returns run length
    private function decodeBlackCodeWord() {
        $current = 0;
        $entry = 0;
        $bits = 0;
        $isT = 0;
    ...
    private function readEOL($isFirstEOL) {
        if ($fillBits == 0) {
            $next12Bits = nextNBits(12);
            if ($isFirstEOL == TRUE && $next12Bits == 0) {
    ...
    private function getNextChangingElement($a0, $isWhite, $ret) {
        ...
        $start = $lastChangingElement > 0 ? $lastChangingElement - 1 : 0;
        if ($isWhite == TRUE) {
    ...
    private function nextNBits($bitsToGet) {
        $b = NULL;//a byte
        $next = NULL; // abyte
        $next2next = NULL; a byte
        $l = itextphp_bytes_getSize($data) - 1;
        $bp = $this->bytePointer;

        if ($fillOrder == 1) {
            $b = itextphp_bytes_getIntValue($bp);

            if ($bp == l) {
                $next = 0x00;
                $next2next = 0x00;
            } else if (($bp + 1) == l) {
                $next = itextphp_bytes_getIntValue($data, $bp + 1);
                $next2next = 0x00;
            } else {
                $next = itextphp_bytes_getIntValue($data, $bp + 1);
                $next2next = itextphp_bytes_getIntValue($data, $bp + 2);
            }
        } else if ($fillOrder == 2) {
            $b = itextphp_bytes_getIntValue(TIFFFaxDecoder::$flipTable, itextphp_bytes_getIntValue($data, $bp) & 0xff);

            if ($bp == l) {
                $next = 0x00;
                $next2next = 0x00;
            } else if (($bp + 1) == l) {
                $next = itextphp_bytes_getIntValue(TIFFFaxDecoder::$flipTable, itextphp_bytes_getIntValue($data, $bp+1)& 0xff);
                $next2next = 0x00;
            } else {
                $next = itextphp_bytes_getIntValue(TIFFFaxDecoder::$flipTable, itextphp_bytes_getIntValue($data, $bp+1) & 0xff);
                $next2next = itextphp_bytes_getIntValue(TIFFFaxDecoder::$flipTable, itextphp_bytes_getIntValue($data, $bp+2) & 0xff);
            }
        } else {
        ...
        $i1 = ($b & TIFFFaxDecoder::$table1[$bitsLeft]) << ($bitsToGet - $bitsLeft);
        $i2 = ($next & TIFFFaxDecoder::$table2[$bitsFromNextByte]) >>> (8 - $bitsFromNextByte);

        $i3 = 0;
        if ($bitsFromNext2NextByte != 0) {
            $i2 <<= $bitsFromNext2NextByte;
            $i3 = ($next2next & TIFFFaxDecoder::$table2[$bitsFromNext2NextByte]) >>>
    ...
    private function nextLesserThan8Bits($bitsToGet) {
        $b = NULL;//a byte
        $next = NULL;// a byte
        $l = itextphp_bytes_getSize($data) - 1;
        $bp = $this->bytePointer;
        ...
        if ($fillOrder == 1) {
            $b = itextphp_bytes_getIntValue($data, $bp);
            if ($bp == l) {
                $next = 0x00;
            } else {
                $next = itextphp_bytes_getIntValue($data, $bp + 1);
    ...
    private function updatePointer($bitsToMoveBack) {
        $i = $bitPointer - $bitsToMoveBack;
...
TIFFFaxDecoder::initializeStatics();
